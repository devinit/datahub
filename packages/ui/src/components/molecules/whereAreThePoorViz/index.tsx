...
import Controls from '../PoorVizControls';
import * as utils from '@devinit/dh-base/lib/utils/poorVizUtils';
...
class Poor extends React.Component {
  ...
  public onScenarioChange(scenario) {
    this.setState({change: true});
  ...
  public createGlobalIconData(iconData) {
    const {level} = this.state;
  ...
  public createRegionalLabels(regionLabelData) {
  ...
  public createGlobalLabels(globalLabelData) {
    const width = 940;
    const svg = this.getSvg();
    const {change} = this.state;
  ...
  public createIcons(iconData) {
    ...
    if (change) {
      ...
      icons.transition().duration(500).delay((d) => {
        ...
        .attr('fill-opacity', 1)
        .attr('transform', (d) => `translate(${d.x} , ${d.y}),scale(${utils.getScale(d.region)})`)
        .attr('fill', (d) => d.color);
    }
    if (level === 'global') {
