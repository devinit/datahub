...
// const sitePath = (config) => path.resolve(config.base, config.paths.source.site);

// const themesPath = (config) => path.resolve(config.base, config.paths.source.themes);
...
//   return R.compose(R.map((line: string) => line.trim()), R.drop(1), filterGroups)(lines);
// };
