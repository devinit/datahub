/* eslint-disable */
// this file is auto generated

export default {
  globalPictureThemes: [
    {
      id: 'poverty',
      name: 'Poverty',
      indicators: [
        {
          id: 'data_series.poor_people_190',
          name: 'Number of people living in extreme poverty',
          heading: 'Number of people living in extreme poverty',
          source: 'Development Initiatives based on World Bank PovcalNet'
        },
        {
          id: 'data_series.poverty_190',
          name: '% of population living in extreme poverty',
          heading:
            'Extreme poverty headcount (% of the population living in extreme poverty)',
          source: 'Development Initiatives based on World Bank PovcalNet'
        },
        {
          id: 'data_series.depth_of_extreme_poverty_190',
          name: 'Depth of extreme poverty (poverty gap)',
          heading:
            'Moderate poverty rate (% of the population living on less than $3.10 a day)',
          source: 'Development Initiatives based on World Bank PovcalNet.'
        },
        {
          id: 'data_series.poverty_310',
          name: '% of population living on less than $3.10 a day',
          heading:
            'Data for those living under $3.10 a day illustrates that while many people are not in extreme poverty (less than $1.90 a day) they are still very poor.',
          source: 'Development Initiatives based on World Bank PovcalNet'
        },
        {
          id: 'data_series.poorest_20_percent',
          name: 'Population in the global P20',
          heading: 'Number of people in the P20',
          source: ''
        },
        {
          id: 'data_series.percent_in_p20_national',
          name: '% of population in the global P20',
          heading: '% of people in the P20',
          source: 'Development Initiatives based on World Bank PovcalNet.'
        },
        {
          id: 'subnational_p20',
          name: '% of people in the P20 (subnational level)',
          heading: '% of population in the global P20',
          source: ''
        },
        {
          id: 'survey_p20',
          name: '% of people in the P20 (survey level)',
          heading: '% of population in the global P20',
          source: ''
        }
      ],
      default_indicator: 'data_series.poor_people_190'
    },
    {
      id: 'vulnerability',
      name: 'Vulnerability',
      indicators: [
        {
          id: 'data_series.fragile_states',
          name: 'Fragile states',
          heading: 'Fragile states',
          source:
            'Development Initiatives based on Fund for Peace Fragile States Index.'
        },
        {
          id: 'data_series.climate_vulnerability',
          name: 'Climate vulnerability',
          heading: 'Climate vulnerability',
          source:
            'Notre Dame Global Adaptation Index (ND-GAIN) vulnerability score.'
        },
        {
          id: 'data_series.human_hazard',
          name: 'Human hazard and exposure score',
          heading: 'Human hazard',
          source: 'Inform (Index for Risk Management) v 3.0.5.'
        },
        {
          id: 'data_series.natural_hazard',
          name: 'Natural hazard and exposure score',
          heading: 'Natural hazard',
          source: 'Inform (Index for Risk Management) v 3.0.5.'
        },
        {
          id: 'data_series.number_of_un_appeals',
          name: 'Number of UN-coordinated appeals',
          heading: 'Number of UN-coordinated appeals per country since 2000',
          source: "Development Initiatives based on UN OCHA's FTS."
        },
        {
          id: 'data_series.forgotten_crisis',
          name:
            "Number of appearances on ECHO's Forgotten Crisis Assessment index",
          heading: 'Forgotten crises',
          source:
            'Development Initiatives based on ECHO Forgotten Crisis Assessment Index.'
        },
        {
          id: 'data_series.displacement',
          name:
            'Number of people displaced due to human hazards by hosting country',
          heading: 'Number of displaced people due to human hazards',
          source:
            'Development Initiatives based on UN High Commissioner for Refugees (UNHCR) and UN Relief and Works Agency for Palestine Refugees in the Near East.'
        }
      ],
      default_indicator: 'data_series.fragile_states'
    },
    {
      id: 'government-finance',
      name: 'Government finance',
      indicators: [
        {
          id: 'data_series.non_grant_revenue_ppp_pc',
          name: 'Government revenue per person (excluding grants), 2015 PPP$',
          heading:
            'Government revenue per person (excluding grants), 2015 constant PPP$',
          source: ''
        },
        {
          id: 'data_series.total_revenue_ppp_pc',
          name: 'Government revenue per person (including grants), 2015 PPP$',
          heading:
            'Government revenue per person (including grants), 2015 constant PPP$',
          source: ''
        },
        {
          id: 'data_series.govt_revenue_pc_gdp',
          name: 'Government revenue (excluding grants), % of GDP',
          heading: 'Government revenue (excluding grants), % of GDP',
          source:
            'Development Initiatives based on IMF Article IV publications and World Economic Outlook.'
        },
        {
          id: 'data_series.total_revenue_percent_gdp',
          name: 'Government revenue (including grants), % of GDP',
          heading: 'Government revenue (including grants), % of GDP',
          source:
            'Development Initiatives based on IMF Article IV publications.'
        },
        {
          id: 'data_series.grants_percent_total_revenue',
          name: 'Grants, % of government revenue',
          heading: 'Grants, % of government revenue',
          source:
            'Development Initiatives based on IMF Article IV publications and World Economic Outlook.'
        },
        {
          id: 'data_series.govt_spend_pc',
          name: 'Government spending per person, 2015 PPP$',
          heading: 'Government spending per person, 2015 constant PPP$',
          source:
            'Development Initiatives based on IMF Article IV publications and World Economic Outlook.'
        }
      ],
      default_indicator: 'data_series.non_grant_revenue_ppp_pc'
    },
    {
      id: 'international-finance',
      name: 'International finance',
      indicators: [
        {
          id: 'data_series.largest_intl_flow',
          name: 'Largest international resource flow',
          heading: 'Largest international resource flow',
          source: ''
        },
        {
          id: 'data_series.intl_resources_total_percent_gdp',
          name: 'Total international resource flows, % of GDP',
          heading: 'Total international resource flows, % of GDP',
          source:
            'Development Initiatives based on numerous sources (see notes for international resource flows to and from developing countries data).'
        },
        {
          id: 'data_series.oda_percent_intl_resources',
          name: 'ODA, % of international resource flows',
          heading: 'ODA, % of international resource flows',
          source:
            'Development Initiatives based on numerous sources (see notes for international resource flows to and from developing countries data).'
        },
        {
          id: 'data_series.fdi_pp',
          name: 'Foreign direct investment per person, US$',
          heading: 'Foreign direct investment per person, US$',
          source:
            'Development Initiatives based on UNCTAD Stat and World Bank Development Indicators.'
        },
        {
          id: 'data_series.fdi_percent_gdp',
          name: 'Foreign direct investment, % of GDP',
          heading: 'Foreign direct investment, % of GDP',
          source:
            'Development Initiatives based on UNCTAD Stat and World Bank Development Indicators.'
        },
        {
          id: 'data_series.remittances_pp',
          name: 'Remittances received per person, US$',
          heading: 'Remittances received per person, US$',
          source: 'Development Initiatives based on World Bank.'
        }
      ],
      default_indicator: 'data_series.largest_intl_flow'
    },
    {
      id: 'international-official-finance',
      name: 'International official finance',
      indicators: [
        {
          id: 'fact.in_oda_gross_2015',
          name: 'Gross ODA received, US$',
          heading: 'Gross ODA received, US$',
          source: 'Development Initiatives based on OECD DAC'
        },
        {
          id: 'fact.in_oda_net_2015',
          name: 'Net ODA received, US$',
          heading: 'Net ODA received, US$',
          source: 'Development Initiatives based on OECD DAC'
        },
        {
          id: 'fact.out_oda_net_2015',
          name: 'Net ODA provided by OECD DAC members, US$',
          heading: 'Net ODA provided by OECD DAC members, US$',
          source: 'Development Initiatives based on OECD DAC.'
        },
        {
          id: 'recipient_profile.oda_per_poor_person_2015',
          name: 'ODA per person living on under $1.90 a day, US$',
          heading: 'ODA per person living on under $1.90 a day, US$',
          source:
            'Development Initiatives based on OECD DAC and World Bank PovcalNet.'
        },
        {
          id: 'fact.oda_percent_gni',
          name: 'Net ODA provided by DAC members, % of GNI',
          heading: 'Net ODA provided by DAC members, % of GNI',
          source: 'OECD DAC'
        },
        {
          id: 'fact.oda_to_ldcs_percent_gni',
          name: 'Gross ODA provided by DAC members to LDCs, % of GNI',
          heading: 'Gross ODA provided by DAC members to LDCs, % of GNI',
          source: 'OECD DAC'
        },
        {
          id: 'data_series.in_oof_gross',
          name: 'Gross other official flows received, US$',
          heading: 'Gross other official flows received, US$',
          source: 'Development Initiatives based on OECD DAC.'
        },
        {
          id: 'data_series.out_oof_net',
          name: 'Net other official flows provided, US$',
          heading: 'Net other official flows provided, US$',
          source: 'Development Initiatives based on OECD DAC.'
        },
        {
          id: 'data_series.out_ssc_net',
          name: 'Net south-south development cooperation provided, US$',
          heading: 'Net south-south development cooperation provided, US$',
          source:
            'Development Initiatives based on OECD DAC and national sources.'
        },
        {
          id: 'data_series.ssc_percent_gni',
          name: 'South-south development cooperation provided, % of GNI',
          heading: 'South-south development cooperation provided, % of GNI',
          source:
            'Development Initiatives based on OECD DAC and national sources.'
        }
      ],
      default_indicator: 'fact.in_oda_gross_2015'
    },
    {
      id: 'humanitarian-finance',
      name: 'Humanitarian finance',
      indicators: [
        {
          id: 'data_series.in_ha',
          name: 'Humanitarian assistance',
          heading: 'Humanitarian assistance',
          source:
            'Development Initiatives based on OECD DAC,UN OCHA FTS and UN CERF.'
        }
      ],
      default_indicator: 'data_series.in_ha'
    },
    {
      id: 'data-revolution',
      name: 'Data revolution',
      indicators: [
        {
          id: 'data_series.latest_census',
          name: 'Latest (planned) population census',
          heading: 'Latest (planned) population census (Africa only)',
          source:
            'UNDESA Population and Vital Statistics Report. Data as available at January 2014.'
        },
        {
          id: 'data_series.latest_hh_survey',
          name: 'Latest (planned) household survey',
          heading: 'Latest (planned) household survey (Africa only)',
          source: 'IHSN Survey Catalog; USAID DHS Program.'
        },
        {
          id: 'data_series.number_of_surveys',
          name: 'Number of poverty surveys in past 10 years',
          heading: 'Number of poverty surveys in past 10 years (Africa only)',
          source: 'World Bank Statistical Capacity Indicators (2014).'
        },
        {
          id: 'data_series.agricultural_census',
          name: 'Latest (planned) agricultural census',
          heading: 'Latest (planned) agricultural census (Africa only)',
          source:
            'FAO World Programme for the Census of Agriculture (WCA): www.fao.org/economic/ess/ess-wca/en/; World Bank World Development Indicators Metadata.'
        },
        {
          id: 'data_series.civil_reg_births',
          name: 'Vital statistics from civil registration (births)',
          heading:
            'Vital statistics from civil registration (births) (Africa only)',
          source:
            'UNDESA Population and Vital Statistics Report. Data as available at Jan 2014; UNSD Coverage of Birth and Death Registration.'
        },
        {
          id: 'data_series.civil_reg_deaths',
          name: 'Vital statistics from civil registration (deaths)',
          heading:
            'Vital statistics from civil registration (deaths) (Africa only)',
          source:
            'WHO African Health Observatory-Civil registration and vital statistics systems.'
        },
        {
          id: 'data_series.health_mis',
          name: 'Health management information system',
          heading: 'Health management information system (Africa only )',
          source: 'Various, including system providers such as DHIS.'
        },
        {
          id: 'data_series.educ_mis',
          name: 'Education management information system',
          heading: 'Education management information system (Africa only)',
          source:
            'UNICEF Country Annual Reports: www.unicef.org/about/annualreport/index_82089.html; UIS Data Quality Assessment Framework (DQAF) output: www.poledakar.com/dqaf/index.php?title=DQAF_Events.'
        },
        {
          id: 'data_series.stat_capacity',
          name: 'Statistical capacity',
          heading:
            'World Bank statistical capacity indicator (overall average 2014) (Africa only',
          source: 'World Bank Statistical Capacity Indicators.'
        }
      ],
      default_indicator: 'data_series.latest_census'
    },
    {
      id: 'forward-looking-oda',
      name: 'Forward looking ODA',
      indicators: [
        {
          id: 'data_series.iati_budgets_gb',
          name: 'UK, DFID planned budgets',
          heading: 'UK Forward-looking Budgets',
          source:
            'Development Initaitives based on UK DFID data downloaded via the IATI Registry on 15 December 2016.'
        },
        {
          id: 'data_series.iati_budgets_ca',
          name: 'Canada, Global Affairs planned budgets',
          heading: 'Canada Forward-looking Budgets',
          source:
            'Development Initaitives based on Canada Global Affairs data downloaded via the IATI Registry on 15 December 2016.'
        },
        {
          id: 'data_series.iati_budgets_dk',
          name: 'Denmark, DANIDA and Foreign Affairs planned budgets',
          heading: 'Denmark Forward-looking Budgets',
          source:
            'Development Initaitives based on Denmark Ministry of Foreign Affairs data downloaded via the IATI Registry on 15 December 2016.'
        },
        {
          id: 'data_series.iati_budgets_nl',
          name: 'Netherlands, Foreign Affairs planned budgets',
          heading: 'Netherlands Forward-looking Budgets',
          source:
            'Development Initaitives based on Netherlands Ministry of Foreign Affairs data downloaded via the IATI Registry on 15 December 2016.'
        },
        {
          id: 'data_series.iati_budgets_be',
          name: 'Belgian Development Cooperation planned budgets',
          heading: 'Belgian Forward-looking Budgets',
          source:
            'Development Initaitives based on Belgian DG Development data downloaded via the IATI Registry on 15 December 2016.'
        }
      ],
      default_indicator: 'data_series.iati_budgets_gb'
    }
  ]
};
