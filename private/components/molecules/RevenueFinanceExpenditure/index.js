...
export default class GovtRFE extends React.Component {
  ...
  constructor(props: Props) {
    ...
    const budgetTypes = makeUnique(
      props.data.filter(d => d.year === props.startYear).map(d => d.budgetType),
    ).sort();
  ...
  updateYear(year: number) {
    const budgetTypes = makeUnique(
      this.props.data.filter(d => d.year === year).map(d => d.budgetType),
    ).sort();
            ...
            <Grid.Column width={5} style={{ paddingRight: 0 }}>
              <CardContainer style={{ paddingLeft: '30px' }}>
                <Timeline
                  ...
                  data={this.props.data
                    .filter(
                      d => d.levels.indexOf(this.state.expenditureLevel) === d.levels.length - 1,
                    )
                    .map(({ year, ...datum }) => ({ year: year.toString(), ...datum }))}
                />
              </CardContainer>
